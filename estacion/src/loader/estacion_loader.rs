...
impl EstacionLoader {
    ...
    pub fn load_ventas_sin_informar(&self) -> Result<HashMap<usize, HashMap<usize, Vec<Venta>>>, Error> {
        let data_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("storage");

        let ventas_sin_informar_path = data_dir.join(format!("ventas_sin_informar_{}.json", self.estacion_id));

        let ventas_sin_informar: HashMap<usize, HashMap<usize, Vec<Venta>>> = self.load_json_nested_hashmap(&ventas_sin_informar_path)?;
        Ok(ventas_sin_informar)
    }

    pub fn save_ventas_sin_informar(&self, ventas: &HashMap<usize, HashMap<usize, Vec<Venta>>>) -> Result<(), Error> {
        let data_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("storage");

        if let Err(e) = std::fs::create_dir_all(&data_dir) {
            return Err(Error::ErrorString(format!("No se pudo crear el directorio {}: {}", data_dir.display(), e)));
        }
        ...
        match File::create(path) {
            Ok(mut file) => {
                match serde_json::to_writer_pretty(&mut file, data) {
                    Ok(_) => {
                        // Asegurar que los datos se escriban completamente al disco
                        match file.flush() {
                            Ok(_) => Ok(()),
                            Err(e) => {
                                let error_msg = format!("No se pudo hacer flush del archivo {}: {}", path.display(), e);
                                eprintln!("{}", error_msg);
                    ...
                    Err(e) => {
                        let error_msg = format!("No se pudo serializar datos en {}: {}", path.display(), e);
                        eprintln!("{}", error_msg);
                        Err(Error::ErrorString(error_msg))
                    }
                }
