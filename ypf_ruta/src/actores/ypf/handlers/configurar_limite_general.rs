...
impl Handler<ConfigurarLimiteGeneral> for YpfRuta {
    ...
    fn handle(&mut self, msg: ConfigurarLimiteGeneral, ctx: &mut Context<Self>) -> Self::Result {
        ...
        actix::spawn(async move {
            let (exito, mensaje) = match gestor_addr.send(ModificarLimiteGeneral {
                ...
                Ok(Ok(())) => {
                    ...
                    (true, format!("Límite general de empresa {} modificado exitosamente a {}", msg.id_empresa, msg.monto))
                }
                Ok(Err(e)) => {
                    eprintln!("YpfRuta {}: Error modificando límite general de empresa {}: {}", ypf_ruta_id, msg.id_empresa, e);
                    (false, format!("Error modificando límite: {}", e))
                }
                Err(e) => {
                    eprintln!("YpfRuta {}: Error enviando mensaje al gestor: {}", ypf_ruta_id, e);
